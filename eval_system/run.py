...
from eval_system.report.html_report import render_html_report
from eval_system.report.markdown_report import HEADLINE_METRIC, render_markdown_report
from eval_system.report.pdf_report import html_to_pdf_bytes
...
def write_report(report: Report, out_dir: Path) -> None:
    ...
    report_html = render_html_report(report, breakdown)
    (out_dir / "report.html").write_text(report_html, encoding="utf-8")
    (out_dir / "report.pdf").write_bytes(html_to_pdf_bytes(report_html))
